...
int initialiseGrid( Grid *g, Grid *cG, Params p );
...
int main( int argc, char** argv ){
	...
	initialiseGrid( &g, &coarseG, p );

	/* Takes dt = min( t_d, 2/(dx)^2, 2/(dy)^2 ) */
	dt = p.t_d;
	if( 2/(g.dx*g.dx) < dt )
		dt = 2/(g.dx*g.dx);
	if( 2/(g.dy*g.dy) < dt )
	...
	while( next_diag <= p.t_f ){
		...
		if( abs(cur_time - next_diag) < 0.0000001  ){
			printResults( cur_time, &g, output );
...
int initialiseGrid( Grid *g, Grid *cG, Params p ){
	...
	for( i=0; i<g->length; i++ ){
		...
		if( index%2 == 0 ){
			cG->T[index/2] 		= tempT;
			cG->T_next[index/2] = tempT;
			cG->E[index/2] 		= tempE;
			cG->E_next[index/2]	= tempE;
			cG->dE[index/2]		= 0;
		}
	}
